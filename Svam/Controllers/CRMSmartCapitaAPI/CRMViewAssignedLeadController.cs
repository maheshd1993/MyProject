...
{
    public class CRMViewAssignedLeadController : ApiController
    { 
        #region Define Entity Class
        niscrmEntities db = new niscrmEntities();
        #endregion

        public HttpResponseMessage Get(Int32 CompanyID, Int32 BranchID, string ProfileName,Int32? LeadOwnerID, Int32? AssignedUserID, string Token)
        {
            ...
            try
            {
                ...
                DataTable GetLeads = DataAccessLayer.GetDataTable("call CRM_GetAssignedLeadList(" + LeadOwnerID + ",'" + MStartDate + "','" + MEndDate + "'," + BranchID + "," + CompanyID + ")");
                ...
                if (AssignedUserID>0)
                {
                    ...
                    if(viewleadsList!=null && viewleadsList.Count>0)
                    {
            ...
            else
            {
                if (Records != string.Empty)
                {
                    ...
                    HttpError err = new HttpError(Records);
                    return Request.CreateResponse(HttpStatusCode.BadRequest, err);
                }
                else
                {
